...
from models import CircleMemberModel, LedgerEntryModel
...
async def test_full_settlement_creates_correct_ledger_entries(
    ...
    resp = await client.post(
        f"/api/cycles/{circle_id}/settle",
        json={
            "member_id": str(seed_circle["charlie_user_id"]),
            "amount_paid": "100.00",
            "debt_id": arrears_id,
        },
    )
...
async def test_partial_settlements_do_not_inflate_late_count(
    ...
    await client.post(
        f"/api/cycles/{circle_id}/settle",
        json={"member_id": str(seed_circle["charlie_user_id"]), "amount_paid": "30.00", "debt_id": arrears_id},
    )
    # Second partial: $30
    await client.post(
        f"/api/cycles/{circle_id}/settle",
    ...
    await client.post(
        f"/api/cycles/{circle_id}/settle",
        json={"member_id": str(seed_circle["charlie_user_id"]), "amount_paid": "40.00", "debt_id": arrears_id},
    )
...
async def test_settle_nonexistent_circle_returns_404(client: AsyncClient):
    resp = await client.post(
        f"/api/cycles/{uuid4()}/settle",
        json={"member_id": str(uuid4()), "amount_paid": "100.00"},
    )
...
@pytest.mark.asyncio
async def test_settle_unknown_member_returns_404(
    client: AsyncClient, seed_circle: dict,
):
    resp = await client.post(
        f"/api/cycles/{seed_circle['circle_id']}/settle",
        json={"member_id": str(uuid4()), "amount_paid": "100.00"},
    )
    assert resp.status_code == 404
...
async def test_settle_no_pending_arrears_returns_404(
    ...
    resp = await client.post(
        f"/api/cycles/{seed_circle['circle_id']}/settle",
        json={"member_id": str(seed_circle["alice_user_id"]), "amount_paid": "100.00"},
    )
...
async def test_zero_amount_rejected_by_schema(
    ...
    resp = await client.post(
        f"/api/cycles/{seed_circle['circle_id']}/settle",
        json={"member_id": str(seed_circle["charlie_user_id"]), "amount_paid": "0.00"},
    )
    assert resp.status_code == 422
...
async def test_sub_cent_amount_rejected_by_schema(
    ...
    resp = await client.post(
        f"/api/cycles/{seed_circle['circle_id']}/settle",
        json={"member_id": str(seed_circle["charlie_user_id"]), "amount_paid": "99.999"},
    )
    assert resp.status_code == 422
...
async def test_member_cannot_settle_another_members_debt(
    ...
    resp = await client.post(
        f"/api/cycles/{seed_circle['circle_id']}/settle",
        json={"member_id": str(seed_circle["bob_user_id"]), "amount_paid": "50.00"},
    )
...
async def test_concurrent_settlements_do_not_double_settle(
    ...
    # Each request gets its own session via the real get_async_db dependency
    # (not the shared test session) to test actual locking behavior.
    app.dependency_overrides[get_current_user_id] = (
        lambda: _make_mock_user_id(seed_circle["charlie_user_id"])
    )
    ...
    async def settle():
        async with AC(transport=transport, base_url="http://test") as c:
            return await c.post(f"/api/cycles/{circle_id}/settle", json=payload)
...
def _make_mock_user_id(user_id: str) -> str:
    return str(user_id)
