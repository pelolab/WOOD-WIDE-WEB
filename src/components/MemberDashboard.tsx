...
const isInflowForMember = (entry: LedgerEntry): boolean =>
  entry.tx_type === 'CONTRIBUTION' || entry.tx_type === 'LATE_CONTRIBUTION';

const amountAsNumber = (entry: LedgerEntry): number => (typeof entry.amount === 'number' ? entry.amount : Number.parseFloat(entry.amount));

const isOutflowForMember = (entry: LedgerEntry): boolean => amountAsNumber(entry) < 0;
...
export const MemberDashboard: React.FC<{ circleId: string; userId: string }> = ({ circleId, userId }) => {
  ...
  const netPosition = useMemo<MemberNetPosition>(() => {
    const contributed = memberEntries
      ...
      .reduce((sum, entry) => sum + amountAsNumber(entry), 0);

    const received = memberEntries
      .filter(isOutflowForMember)
      .reduce((sum, entry) => sum + Math.abs(amountAsNumber(entry)), 0);
    ...
    return {
      ...
      net: Number((contributed + penalties - received).toFixed(2)),
              ...
              <TableBody>
                {memberEntries.map((entry) => (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell>${amountAsNumber(entry).toFixed(2)}</TableCell>
