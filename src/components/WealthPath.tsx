...
import { fetchMemberAnalytics, fetchSovereigntyProjection } from '../lib/api';
...
export const WealthPath: React.FC<WealthPathProps> = ({ circleId, userId }) => {
  ...
  const [sovereigntyProjection, setSovereigntyProjection] = useState<SovereigntyProjection>({
    groupLiquidity: 0,
    equivalentLoanPrincipal: 0,
    equivalentLoanInterest: 0,
  ...
  useEffect(() => {
    ...
    const loadProjection = async () => {
      try {
        const projection = await fetchSovereigntyProjection(circleId, userId);
        if (alive) setSovereigntyProjection(projection);
      } catch {
        if (alive) {
          setSovereigntyProjection({
            groupLiquidity: 0,
            equivalentLoanPrincipal: 0,
            equivalentLoanInterest: 0,
            totalLoanRepayment: 0,
